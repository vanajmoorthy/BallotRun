...
public final class GameStateBase extends GameState {
    ...
    private final GestureDetector buttonEqip = new GestureDetector((sketch, hitbox, hasHover, hasClick) -> {
        ...
        sketch.fill(
                hasHover && (isItemEquipped || inventoryHasSpace) ? Colors.darkGray.light : Colors.darkGray.primary);
        sketch.noStroke();
        sketch.rect(Constants.Screen.Base.selection.x + Constants.Screen.Base.padding,
                Constants.Screen.Base.selection.y + Constants.Screen.Base.selection.height
                        - Constants.Screen.Base.padding - 40,
                Constants.Screen.Base.selection.width - 2 * Constants.Screen.Base.padding, 40, 10);

        sketch.textAlign(PApplet.CENTER, PApplet.CENTER);
        sketch.fill(Colors.white);
        sketch.text(isItemEquipped ? "UNEQUIP" : inventoryHasSpace ? "EQUIP" : "FULL INVENTORY",
                Constants.Screen.Base.selection.x + Constants.Screen.Base.selection.width / 2,
                Constants.Screen.Base.selection.y + Constants.Screen.Base.selection.height
                        - Constants.Screen.Base.padding - 20);
    }, (sketch, button) -> {
        boolean inventoryHasSpace = !player.isInventoryFull();
        boolean isItemEquipped = selectedItem != null && player.getInventory().contains(selectedItem);
        ...
        if (isItemEquipped) {
            ...
            new PVector(Constants.Screen.Base.selection.x + Constants.Screen.Base.padding,
                    Constants.Screen.Base.selection.y + Constants.Screen.Base.selection.height
                            - Constants.Screen.Base.padding - 40),
            new PVector(Constants.Screen.Base.selection.width - 2 * Constants.Screen.Base.padding, 40)));

    private final GestureDetector buttonPlay = new GestureDetector((sketch, hitbox, hasHover, hasClick) -> {
        if (hasHover)
            cursor = PApplet.HAND;

        sketch.fill(hasHover ? Colors.darkGray.light : Colors.darkGray.primary);
        sketch.noStroke();
        sketch.rect(Constants.Screen.Base.selection.x + Constants.Screen.Base.padding,
                Constants.Screen.Base.selection.y + Constants.Screen.Base.selection.height
                        - Constants.Screen.Base.padding - 40,
        ...
        sketch.textAlign(PApplet.CENTER, PApplet.CENTER);
        sketch.fill(Colors.white);
        sketch.text("PLAY", Constants.Screen.Base.selection.x + Constants.Screen.Base.selection.width / 2,
                Constants.Screen.Base.selection.y + Constants.Screen.Base.selection.height
                        - Constants.Screen.Base.padding - 20);
    }, (sketch, button) -> {
        didStartGame = true;
    ...
    private final GestureDetector buttonClose = new GestureDetector((sketch, hitbox, hasHover, hasClick) -> {
        int width = 30;
        int height = 30;
        ...
        sketch.fill(hasHover ? Colors.darkGray.light : Colors.darkGray.primary);
        sketch.noStroke();
        sketch.rect(x, y, width, height, 5);

        sketch.stroke(Colors.white);
        sketch.strokeWeight(4);
        sketch.line(x + padding, y + padding, x + width - padding, y + height - padding);
        sketch.line(x + padding, y + width - padding, x + height - padding, y + padding);
    }, (sketch, button) -> {
        selectedItem = null;
    }, new GestureDetector.Hitbox(new PVector(Constants.Screen.width - 2 * Constants.Screen.Base.padding - 30,
            Constants.Screen.Base.selection.y + Constants.Screen.Base.padding), new PVector(30, 30)));
    ...
    public GameStateBase(Player player, List<Item> items) {
        ...
        for (int i = 0; i < items.size(); i++) {
            ...
            buttons.add(new GestureDetector((sketch, hitbox, hasHover, hasClick) -> {
                if (hasHover)
                    cursor = PApplet.HAND;
                ...
                try {
                    sketch.image(sketch.loadImage(ResourceUtils
                            .getFile("classpath:textures/" + item.getType().getId() + ".png").getAbsolutePath()
                    // new ClassPathResource("classpath:textures/" + item.getType().getId() +
                    // ".png").getFilename()
                    ), 10 + offsetX + 9, 10 + offsetY + 9, 32, 32);
                ...
                selectedItem = item;
            }, new GestureDetector.Hitbox(new PVector(10 + offsetX, 10 + offsetY), new PVector(50, 50))));
        }

        for (int j = 0; j < buttonsInventory.length; j++) {
            final int i = j;
            final int offsetY = i * 55 + 50;
            buttonsInventory[i] = new GestureDetector((sketch, hitbox, hasHover, hasClick) -> {
                ...
                sketch.fill(hasHover && item != null ? Colors.darkGray.light : Colors.darkGray.primary);
                sketch.noStroke();
                sketch.rect(Constants.Screen.Base.selection.x + Constants.Screen.Base.padding,
                        Constants.Screen.height - 3 * Constants.Screen.Base.padding - 40 - offsetY, 50, 50, 10);

                sketch.noFill();
                sketch.stroke(item != null ? item.getType().getRarity().getColor() : Colors.darkGray.lighter);
                sketch.strokeWeight(2);
                sketch.rect(Constants.Screen.Base.selection.x + Constants.Screen.Base.padding + 5,
                        Constants.Screen.height - 3 * Constants.Screen.Base.padding - 40 - offsetY + 5, 40, 40, 5);
                ...
                if (item != null) {
                    ...
                    try {
                        sketch.image(
                                ...
                                Constants.Screen.height - 3 * Constants.Screen.Base.padding - 40 - offsetY + 9, 32, 32);
                    } catch (FileNotFoundException e) {
                        e.printStackTrace();
                    }
                }

                sketch.fill(item != null ? item.getType().getRarity().getColor() : Colors.darkGray.lighter);
                sketch.noStroke();
                sketch.textAlign(PApplet.LEFT, PApplet.CENTER);
                sketch.text(item != null ? item.getType().getDisplayName() : "Empty",
                ...
                if (hasHover && item != null) {
                    ...
                    sketch.stroke(Colors.white);
                    sketch.strokeWeight(4);
                    sketch.line(Constants.Screen.Base.selection.x + Constants.Screen.Base.padding + 15,
    ...
    public GameState draw(PApplet sketch) {
        ...
        sketch.noStroke();
        sketch.rect(Constants.Screen.Base.selection.x, Constants.Screen.Base.selection.y,
                Constants.Screen.Base.selection.width, Constants.Screen.Base.selection.height, 15);
    ...
    private void drawInventory(PApplet sketch) {
        ...
        for (Attribute attribute : Attribute.values()) {
            ...
            sketch.text(attribute.getSymbol(), Constants.Screen.Base.selection.x + Constants.Screen.Base.padding,
            ...
            sketch.text(Math.round(value), Constants.Screen.Base.selection.x + Constants.Screen.Base.padding + offsetX,
                    Constants.Screen.Base.selection.y + 2 * Constants.Screen.Base.padding + offsetY);
    ...
    private void drawSelectedItem(PApplet sketch) {
        ...
        sketch.noStroke();
        sketch.rect(Constants.Screen.Base.selection.x + Constants.Screen.Base.padding,
                Constants.Screen.Base.selection.y + Constants.Screen.Base.padding, itemIconSize, itemIconSize, 10);

        sketch.noFill();
        sketch.stroke(selectedItem.getType().getRarity().getColor());
        sketch.strokeWeight(2);
        sketch.rect(Constants.Screen.Base.selection.x + Constants.Screen.Base.padding + 5,
                Constants.Screen.Base.selection.y + Constants.Screen.Base.padding + 5, itemIconSize - 10,
                itemIconSize - 10, 5);

        // sketch.image(
        // imageController.get(sketch, "classpath:textures/" +
        // selectedItem.getType().getId() + ".png"),
            ...
            sketch.image(
                    sketch.loadImage(
                            ResourceUtils.getFile("classpath:textures/" + selectedItem.getType().getId() + ".png")
                                    .getAbsolutePath()),
                    Constants.Screen.Base.selection.x + Constants.Screen.Base.padding + 10,
                    Constants.Screen.Base.selection.y + Constants.Screen.Base.padding + 10, itemIconSize - 20,
                    itemIconSize - 20);
        } catch (FileNotFoundException e) {
            e.printStackTrace();
        }
        ...
        sketch.noStroke();
        sketch.text(selectedItem.getType().getDisplayName(),
        ...
        sketch.text(selectedItem.getType().getRarity().getDisplayName().toUpperCase(),
                Constants.Screen.Base.selection.x + 2 * Constants.Screen.Base.padding + itemIconSize,
        ...
        List<AttributeModifier> modifiers = itemAttributes.values().stream()
                .sorted((a, b) -> a.getAttribute().compareTo(b.getAttribute())).collect(Collectors.toList());
        int i = 0;
        sketch.textSize(20);
        for (AttributeModifier modifier : modifiers) {
            sketch.fill(modifier.getAttribute().getColor());
            sketch.text(modifier.getAttribute().getSymbol(),
            ...
            sketch.text(modifier.getAttribute().getDisplayName(),
            ...
            sketch.fill(Colors.white);
            sketch.text(": ", Constants.Screen.Base.selection.x + Constants.Screen.Base.padding + offset,
                    Constants.Screen.Base.selection.y + 2 * Constants.Screen.Base.padding + itemIconSize + 22 * i);

            offset += sketch.textWidth(": ");
            sketch.fill(modifier.getValue() >= 0 ? Colors.white : Colors.red.darker);
            sketch.text(Math.round(modifier.getValue()),
                    Constants.Screen.Base.selection.x + Constants.Screen.Base.padding + offset,
                    Constants.Screen.Base.selection.y + 2 * Constants.Screen.Base.padding + itemIconSize + 22 * i);
        ...
        // lore
        selectedItem.getType().getLore().draw(sketch, Constants.Screen.Base.selection.x + Constants.Screen.Base.padding,
                Constants.Screen.Base.selection.y + 2 * Constants.Screen.Base.padding + itemIconSize + 22 * (i + 1),
                Constants.Screen.Base.selection.width - 2 * Constants.Screen.Base.padding, 20);
