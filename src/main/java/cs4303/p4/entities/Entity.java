    ...
    public void move(Tile[][] levelGrid) {
        ...
        if (v_x > 0) {
            for (int i = tile_x_max + 1; i <= (int) ((x_max + v_x) / Constants.TILE_SIZE); i++) {
                if (
                    i >= 0 &&
                    i < levelGrid[0].length &&
                    (
                        tile_y_min < levelGrid.length && tile_y_min >= 0 && levelGrid[tile_y_min][i].getType() == TileType.PLATFORM ||
                        tile_y_max < levelGrid.length && tile_y_max >= 0 && levelGrid[tile_y_max][i].getType() == TileType.PLATFORM
                    )
                ) {
        ...
        if (v_y > 0) {
            for (int i = tile_y_max + 1; i <= (int) ((y_max + v_y) / Constants.TILE_SIZE); i++) {
                if (
                    ...
                    i < levelGrid.length &&
                    (
                        tile_x_min < levelGrid[0].length && tile_x_min >= 0 && levelGrid[i][tile_x_min].getType() == TileType.PLATFORM ||
                        tile_x_max < levelGrid[0].length && tile_x_max >= 0 && levelGrid[i][tile_x_max].getType() == TileType.PLATFORM
                    )
                ) {
